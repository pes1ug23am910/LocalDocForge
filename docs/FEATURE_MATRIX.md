...
| OCR PDF | ✅ when OCRmyPDF ≥17.8.1 + Tesseract ≥4.1.1 (except exact 5.4.0) + Ghostscript are available | locked OCRmyPDF + separately installed Tesseract/Ghostscript | Lib, CLI, API | `test_ocr_ops.py`, `test_cli.py`, `test_api.py`, `test_registry.py`, `test_documentation_consistency.py` | Default refuses any existing text layer; `--skip-text` handles mixed PDFs and `--force-ocr` critically warns that every page is rasterized/re-encoded. Every output has `ocr-text-approximate`; optional sidecars are strict UTF-8, omit pre-existing skipped-page text, and preserve documented form-feed page separators plus engine control markers. OCRmyPDF runs with one worker and bounded time/resources; output must pass full PDF validation and PDFium text extraction. LocalDocForge never directly invokes or bundles AGPL Ghostscript |
...
| Registry-derived agent brief | ✅ | `CAPABILITY_SPECS` + live capability probes | CLI | `test_agent_brief.py`, `test_cli.py::TestAgentBrief`, `test_documentation_consistency.py` | Markdown/JSON is emitted only on stdout; every implemented command appears with live availability and one-line usage, while unimplemented capabilities are structurally excluded. The Ghostscript live gate may create bounded synthetic files only under a validated local temp root and attempts cleanup; cleanup failure makes the gate unavailable but OS-locked residue may remain. The command requires a discoverable source checkout because the writable feedback log is intentionally not packaged; no API/UI endpoint |
